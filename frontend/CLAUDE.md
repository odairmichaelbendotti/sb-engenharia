...
7. **`ObraDetailPanel.tsx` foi redesenhado em 2026-09-02** (o usuário achou a tradução literal das imagens de referência "estranha" — pediu para usar a skill `design` e propor algo diferente, com liberdade total de estrutura). O resultado, aprovado pelo usuário via canvas publicado como Artifact, trocou o header escuro por um header claro (tokens normais do app) com uma faixa fina na cor do contrato, e reorganizou o conteúdo em 3 abas — **Geral** (Sobre a obra + Execução Financeira + Contrato), **Notas Fiscais**, **Cronograma** — voltando ao modelo de abas da primeira versão depois de uma iteração intermediária em accordion/scroll único que o usuário achou bagunçada ("muita coisa para scroll"). Campos ainda não implementados (`E-PAG`, `Aditivo Total`, `% Execução Física`, `Variação Físico/Financeiro`, histórico de aditivos de prazo na aba Cronograma) usam o componente `<PendingBadge />` — um selo discreto "Em breve" (cinza, borda tracejada, ícone de relógio) — no lugar do vermelho+negrito da primeira versão (componente `<Missing />`, removido). Não "completar" esses campos com dado inventado — são placeholders esperando modelagem futura (aditivos de contrato, avanço físico de obra). O canvas de referência do redesign está em `https://claude.ai/code/artifact/85fea2ae-a376-4783-8d2c-c9ae094c07ca` (privado, do usuário) — útil para comparar visualmente antes de mexer de novo no layout.
